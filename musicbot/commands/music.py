...
class Music(commands.Cog):
    ...
    @commands.command(name="p", description=config.HELP_YT_LONG, help=config.HELP_YT_SHORT, aliases=['play', 'yt', 'pl'])
    async def _play_song(self, ctx, *, track: str):
        ...
        await ctx.send("Join our Discord Channel.")
        await ctx.send("https://discord.gg/AdWGqMRrqV")
    ...
    @commands.command(name='q', description=config.HELP_QUEUE_LONG, help=config.HELP_QUEUE_SHORT,
    ...
    async def _queue(self, ctx):
        ...
        if config.MAX_SONG_PRELOAD > 10:
            config.MAX_SONG_PRELOAD = 10

        embed = discord.Embed(title="Queue [{}]".format(len(playlist.playque)), color=config.EMBED_COLOR)
    ...
    @commands.command(name='next', description=config.HELP_SKIP_LONG, help=config.HELP_SKIP_SHORT, aliases=['s'])
    ...
    @commands.command(name='i', description=config.HELP_SONGINFO_LONG, help=config.HELP_SONGINFO_SHORT, aliases=["np"])
    ...
    async def _volume(self, ctx, *args):
        ...
        if len(args) == 0:
            await ctx.send("Current volume: {}% ".format(utils.guild_to_audiocontroller[ctx.guild]._volume))
            return
