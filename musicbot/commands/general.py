...
class General(commands.Cog):
    ...
    async def _reset(self, ctx):
        ...
        await ctx.send("{} Connected to {}".format("", ctx.author.voice.channel.name))
    ...
    async def _change_channel(self, ctx):
        ...
        if vchannel == ctx.author.voice.channel:
            await ctx.send("{} Already connected to {}".format("", vchannel.name))
            return
        ...
        await ctx.send("{} Switched to {}".format("", ctx.author.voice.channel.name))

    @commands.command(name='birger', description=config.HELP_BIRGER_LONG, help=config.HELP_BIRGER_SHORT)
    async def _birger(self, ctx):
        await ctx.send("My name is Birger.")

    @commands.command(name='about', description=config.HELP_BIRGER_ABOUT_LONG, help=config.HELP_BIRGER_ABOUT_SHORT)
    async def _about(self, ctx):
        await ctx.send("Birgeeeeeeeeeeeeeeeer")

    @commands.command(name='explain', description=config.HELP_BIRGER_EXPLAIN_LONG, help=config.HELP_BIRGER_EXPLAIN_SHORT)
    async def _explain(self, ctx):
        await ctx.send("Birgeeeeeeeeeeeeeeeer raaah!!!")
